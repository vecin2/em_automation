...
def test_it_throws_exception_when_run_with_em_context_and_env_var_is_not_set(
    fs, app_runner
):
    fs.create_dir("/my_templates")
    with pytest.raises(EnvVarNotFoundException) as excinfo:
        app_runner.using_templates_under("/my_templates").saveAndExit().run_prod()
    assert "path of your current EM project" in str(excinfo.value)


def test_it_throws_exception_when_run_with_em_context_and_em_path_does_not_exist(
    app_runner, fs
):
    fs.create_dir("/my_templates")
    with pytest.raises(ValueError) as excinfo:
...
def test_returns_empty_when_no_template_selected(app_runner, fs):
    fs.create_dir("/my_templates")
    app_runner.using_templates_under(
        "/my_templates"
...
def test_asks_for_template_until_valid_entry(app_runner, fs):
    ...
    app_runner.using_templates_under("/templates").select_template(
        "abc", {}
    ).saveAndExit().run().assert_rendered_sql("").assert_all_input_was_read()


def test_computes_templates_path_from_prj_path(app_runner, fs):
    app_runner.with_emproject_under("/em/prj").and_prj_built_under(
        "/em/prj"
    ).add_template("say_hello.sql", "hello!").select_template(
        "1. say_hello.sql", {}
    ).saveAndExit().run().assert_rendered_sql(
        "hello!"
...
def test_computes_templates_path_from_current_path(app_runner):
    app_runner.and_prj_built_under("/em/prj").from_current_dir("/em/prj").add_template(
        "say_bye.sql", "bye!"
    ).select_template("say_bye.sql", {}).saveAndExit().run().assert_rendered_sql(
        "bye!"
    ).assert_all_input_was_read()
