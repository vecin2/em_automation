...
def test_include_templates():
    ...
    for field, value in ast.iter_fields():
        assert "body" == field
        assert (
            "Include(template=Const(value='add_process_descriptor.sql'), "
            + "with_context=True, "
...
def test_template_body_structure():
    ...
    for field, value in ast.iter_fields():
        ...
        assert (
            "[Output(nodes=[TemplateData(data='Hello '), Name(name='name', ctx='load'), TemplateData(data='!')])]"
            == str(value)
        )
...
def test_pipe_default_descripion_filters():
    ast = env.parse(
        "Hello {{ name | default ('Mundo') | description ('World in english') }}!"
    )
    body_string = (
        "Output(nodes=[TemplateData(data='Hello '), "
        + "Filter(node=Filter("
...
def test_globals():
    ...
    body_string = (
        "Assign(target=Name(name='name', ctx='store'), "
