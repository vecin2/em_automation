...
### Example: Add Activity To Perspective
...
This works great long as the user knows that verbs on the left hand side verbs in the left hand side with the word "activities" 
 things about this scenario as "adding activities to perspective" then he'll be able to find it really quick.
...
This is why it is importatn how the templates are named, and it is reponsability of the library to choose appropiate names and to define name conventions that help finding templates quickly. For the current library some general naming rules are defined [here](dabl)
...
This is often used in projects, when the developer knows that a certain SQLTask touches a specific database table.
For example, a user wants to add a verb to an entity and he might know that "EVA_VERB" is one of tables involved. He could use a search tool to search on the templates folder by "EVA_VERB" and that will retrieve all the templates files that touch this table. Then, from the template file name we should be able to identify which template to use.
