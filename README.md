...
<p>sqktask is command line application that helps users generating templates, in particular, SQL scripts which are commonly used within EM developement.</p>
<p>It allows the user to select from a list of templates and once the template is selected it will start prompting to enter the template values.</p>
<p>Templates are written using <a href="http://jinja.pocoo.org/">jinja templates syntax</a>  and they should be designed to provide good information to users when entering values and to minimize user interactions, avoiding asking for values that could be computed.</p>
...
<p>This section shows a basic use case of how sql scripts are generated:</p>
<ul>
...
<li>Select the template, and the system should prompt the user to enter the template values:</li>
</ul>
...
<ul>
<li>Assuming the user enters <code>Customer</code>, <code>Home</code> and <code>indentifyCustomer</code>, the following will be printed out:</li>
</ul>
<pre class=" language-sql"><code class="prism  language-sql">	<span class="token keyword">UPDATE</span> EVA_CONTEXT_VERB_ENTRY
...
<h3 id="creating--a-sql-task">Creating  a SQL Task</h3>
<pre><code>sqltask -d modules/ABCustomer/sqlScripts/oracle/updates/Project_R1_0_0/add_policy_to_Customer_table
</code></pre>
<p>It generates and SQL task under the current <code>EM_CORE_HOME</code>, which includes both files <code>tableData.sql</code> and <code>update.sequence</code>.</p>
<h3 id="exiting-the-application">Exiting the application</h3>
<p>At any point press <code>Ctrl+c</code> or <code>Ctrl+d</code> to exit.  When using Gitbash it might require to hit <code>Enter</code> after  <code>Ctrl+c</code> or <code>Ctl+d</code>.</p>
...
<ul>
<li>
<p>Create a file with <code>.sql</code> extension (only sql files are read) under <code>$SQL_TEMPLATES_PATH</code></p>
</li>
<li>
<p>Create a shortcut with the same name pointing to the previous file under <code>$SQL_TEMPLATES_PATH/menu</code>. Make sure the shorcut name matches the template name otherwise it will not show up in the menu when running the application.</p>
</li>
</ul>
<h4 id="hidding-a-template">Hidding a Template</h4>
<p>Template can be hidden by simply not creating the shorcut<br>
.<br>
It is good practice to reuse templates to avoid duplicating SQL code. Therefore a template can be created to support others but it doesn’t need to be displayed as it will always be invoked through a wrapping template.</p>
<h1 id="user-installation">User installation</h1>
<p>Make sure you have the following:</p>
<ul>
<li>
<p>Install <a href="https://www.python.org/ftp/python/3.7.0/python-3.7.0.exe">python3</a>.</p>
</li>
<li>
<p>Add the python installation folder to the system path</p>
</li>
<li>
<p>Copy the template folder to some location in you filesystem. For example under your current EM project.</p>
</li>
<li>
<p>Add the following environment variables:</p>
<ul>
<li><code>PYTHON_HOME</code> pointing to python installation folder.</li>
<li><code>EM_CORE_HOME</code> pointing to your EM project, e.g. <code>/opt/em/projects/gsc</code></li>
<li><code>SQL_TEMPLATES_PATH</code> pointing to the folder containing the sql templates. e.g. <code>/opt/em/projects/my_project/sql_templates</code></li>
</ul>
</li>
<li>
<p>Install <a href="https://test.pypi.org/project/sqltask/">sqltask</a> by typing the command line (if you have multiple versions of python installed make sure you are using version 3):<br>
<code>python3 -m pip install --extra-index-url https://test.pypi.org/simple/ sqltask</code></p>
</li>
</ul>
<p>The command above should install all the required packages including <a href="http://jinja.pocoo.org/">jinja2 templates</a>. But if you still get issues when running sqltask where it can’t find jinja you can install it manually by running<br>
<code>python3 -m pip install Jinja2</code>.</p>
<h3 id="windows-console-tools">Windows Console Tools</h3>
<p>If you find Windows console doesn’t provide with easy path autocompletion, easy copy and paste, etc. You can  look at other options:</p>
<ul>
<li><a href="http://mridgers.github.io/clink/">Clink</a>: very light weight tool which add a set features yo you Windows console.</li>
...
<p>How template values are prompted to the user is determined entirely by how the template is written. So having a set of well design template is the key for the application to work well.</p>
<p>The syntax is defined by python jinja templates. Check their <a href="http://jinja.pocoo.org/docs/2.10/templates/">template Designer Documentation</a>.</p>
<p>When design templates consider:</p>
<ul>
<li>A value shouuld be prompted with enough information so the user knows how to populate it.</li>
<li>When possible provide a subset of values for the user to pick from.</li>
<li>Users should NOT be prompted a value if the value can be computed from some other value.</li>
<li>Templates should be reused so if one DB table changes then it it is easy to update the templates to reflect that.</li>
<li>Review existing templates or consult this documentation to undestand what filters and templates are available.</li>
</ul>
...
<h2 id="list-of-global-functions">List of Global Functions</h2>
<p>To the existin <a href="http://jinja.pocoo.org/docs/2.10/templates/#builtin-globals">list of jinja builtin global functions</a> we have added the following:</p>
...
<p>Globals functions can easily implemented by adding the function to the <code>globals.py</code> module and then adding the function to the environment when the environment is created:<br>
<code>self.env.globals['camelcase'] = camelcase</code></p>
