...
from sqltask.exceptions import CCAdminException
...
class CCAdmin(object):
    ...
    def show_config(self, params):
        sqltask.logger.debug("Running ccadmin show-config " + params)
        result = self._run_ccadmin("show-config " + params)
        sqltask.logger.debug("End Running ccadmin show-config")
        return result
