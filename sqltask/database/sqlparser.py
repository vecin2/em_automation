...
class RelativeIdLoader(object):
    ...
    def keyname_by_id(self, keyset, id):
        return self.db.find(
            "SELECT KEYNAME FROM CCADMIN_IDMAP WHERE KEYSET ='"
            + keyset
            + "' AND ID ="
            + str(id)
        )["KEYNAME"]
    ...
    def _fetch_keyset(self, keyset):
        result = self.db.fetch(
            "SELECT KEYNAME,ID FROM CCADMIN_IDMAP WHERE KEYSET ='" + keyset + "'"
        )
    ...
    def _handle_duplicate_keys(self, keyset, keyname, id):
        ...
        error_msg = (
            "Found multiple records for keyset '"
            + keyset
            + "'"
            + " and keyname '"
            + keyname
            + "' combination. There should"
    ...
    def generate_id(self, keyset, keyname):
        query = (
            "SELECT ID FROM CCADMIN_IDMAP where keyset ='{}' order by id desc".format(
                keyset
            )
        )
